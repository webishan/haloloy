...
// Helper function to calculate 15% cashback on 1500 Taka discount
const calculateCashbackIncome = (discountAmount: number): number => {
  if (discountAmount >= 1500) {
    return discountAmount * 0.15; // 15% cashback
  }
  return 0;
};

// Helper function to calculate 2% referral commission per 1000 Taka
const calculateReferralIncome = (salesAmount: number): number => {
  return Math.floor(salesAmount / 1000) * (1000 * 0.02); // 2% per 1000 Taka
};

// Helper function to calculate 1% royalty from total sales
const calculateRoyaltyIncome = (totalSales: number): number => {
  return totalSales * 0.01; // 1% royalty
};
...
// Process cashback income (15% on 1500 Taka discount)
router.post('/income/cashback', async (req, res) => {
  try {
    const merchantId = await getMerchantId(req.user.id);
    const { discountAmount, saleId } = req.body;
    
    if (!discountAmount || discountAmount < 0) {
      return res.status(400).json({ error: 'Invalid discount amount' });
    }
    
    const cashbackAmount = calculateCashbackIncome(discountAmount);
    if (cashbackAmount <= 0) {
      return res.status(400).json({ 
        error: 'Minimum 1500 Taka discount required for cashback' 
      });
    }
    ...
    await storage.createMerchantIncome({
      merchantId,
      incomeType: 'cashback_15_percent',
      amount: cashbackAmount.toString(),
      sourceAmount: discountAmount.toString(),
      description: `15% cashback on ${discountAmount} Taka discount`,
      referenceId: saleId
    });
    ...
    await storage.createWalletTransaction({
      ...
      amount: cashbackAmount.toString(),
      description: `15% cashback income from discount`,
      balanceAfter: newIncomeBalance.toString()
    });
    ...
    res.json({ 
      ...
      cashbackAmount,
      newIncomeBalance,
      message: 'Cashback income processed successfully' 
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
