...
import { db } from "./db";
import { pointDistributions, admins } from "../shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/generate-points', authenticateToken, authorizeRole(['global_admin']), async (req, res) => {
    try {
      ...
      let adminResult = await db.select().from(admins).where(eq(admins.userId, req.user.userId)).limit(1);
      let admin = adminResult[0];
      if (!admin) {
        ...
        admin = { pointsBalance: 0, totalPointsReceived: 0 };
      }
      ...
      const transaction = await db.insert(pointDistributions).values({
        fromUserId: req.user.userId,
  ...
  app.post('/api/admin/distribute-points', authenticateToken, authorizeRole(['global_admin', 'local_admin']), async (req, res) => {
    try {
      ...
      const senderAdminResult = await db.select().from(admins).where(eq(admins.userId, req.user.userId)).limit(1);
      const senderAdmin = senderAdminResult[0];
      ...
      if (req.user.role === 'global_admin') {
        ...
        const receiverAdmin = await db.select().from(admins).where(eq(admins.userId, toUserId)).limit(1);
        if (receiverAdmin.length > 0) {
          await db.update(admins)
            .set({
              pointsBalance: (receiverAdmin[0].pointsBalance || 0) + pts,
              totalPointsReceived: (receiverAdmin[0].totalPointsReceived || 0) + pts
            })
      ...
      const distribution = await db.insert(pointDistributions).values({
        ...distributionData,
        points: pts
      }).returning();
